...
def get_train_test_lists(version='01'):
    ...
    # Build the train list. Extrqa step to remove the class index.
...
def move_files(file_groups):
    ...
    for group, videos in file_groups.items():
        ...
        for video in videos:
            ...
            # Get the parts.
            parts = video.split('/')  # init: video.split(os.path.sep)
            print(parts)
            classname = parts[0]
            filename = parts[1]
            ...
            filename_ucf101 = 'UCF-101/' + classname + '/' + filename
            file_t = group + '/' + classname
            # Check if we have already moved this file, or at least that it exists to move.
            # if not os.path.exists(filename_t):
            #     print("Can't find %s to move. Skipping." % (filename))
            #     continue

            # Move it. from UCF101 to train/test folder
            # dest = 'train/YoYo/v_YoYo_g07_c03.avi'
            # filename = 'v_YoYo_g07_c03.avi'
            # is it necessary to rename them?
            # dest = os.path.join(group, classname, filename)
            shutil.copy(filename_ucf101, file_t)
            print("Moving %s to %s" % (filename, file_t))
